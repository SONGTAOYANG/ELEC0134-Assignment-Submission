...
from A.info import INFO, HOMEPAGE
...
class MedMNIST(Dataset):
    ...
    def __init__(self,
        ...
        if not os.path.exists("./Datasets/pneumoniamnist.npz"):
...
class MedMNIST3D(MedMNIST):
    ...
    def montage(self, length=20, replace=False, save_folder=None):
        ...
        from A.utils import montage3d, save_frames_as_gif
        n_sel = length * length
        sel = np.random.choice(self.__len__(), size=n_sel, replace=replace)
